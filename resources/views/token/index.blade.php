@extends('layouts.app')
@section('page-title')
    {{ ucfirst(__('validation.attributes.tokens')) }}
@stop
@section('content')
    <div class="content container-fluid">
        <div class="row">
            <div class="col-xs-7">
                <h4 class="page-title">{{ ucfirst(__('validation.attributes.tokens'))}}</h4>
            </div>
            <div class="col-xs-5 text-right m-b-30">
                @can('token', auth()->user()->member)
                    <a href="#" class="btn btn-default disabled">{{ ucfirst(__('validation.attributes.soldLeft')). ' : ' . auth()->user()->member->company->premium->sold }}</a>
                                            ...
                                            <ul class="dropdown-menu pull-right">
                                                <li><a href="#" data-toggle="modal"
                                                       data-target="#delete_token{{$token->id}}"><i
                                                            class="fa fa-trash-o m-r-5"></i> {{ ucfirst(__('validation.attributes.delete')) }}
                                                    </a></li>
                                            </ul>
                                        </div>
                                    </td>
